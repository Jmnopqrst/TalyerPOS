...
export function SuperAdminConsole({ user, settings, onSignOut }: { user: UserAccount; settings?: ReceiptSettings; onSignOut: () => void }) {
  ...
  async function loadConsoleData() {
    try {
      ...
      setBackupForm({
        backupSchedule: next.settings.backup_schedule,
        ...
        backupRetentionCount: Number(next.settings.backup_retention_count || 10)
      });
          ...
          <div className="content-grid">
            ...
            <section className="panel">
              <div className="panel-head">
                ...
                <Badge>{consoleData.settings.backup_schedule}</Badge>
              </div>
              <div className="form-grid">
                <label className="field">
                  Frequency
                ...
                <label className="field">
                  Backups to Keep
                  <input type="number" min={1} max={365} value={backupForm.backupRetentionCount} onChange={(event) => setBackupForm({ ...backupForm, backupRetentionCount: Math.max(1, Math.min(365, Number(event.target.value) || 1)) })} />
                </label>
              ...
              <div className="detail-grid backup-monitor-grid">
                ...
                <span>Retention <b>Keep last {consoleData.settings.backup_retention_count} backups</b></span>
                ...
                {consoleData.backupHistory.map((backup) => (
                  <article className="super-admin-log-card" key={backup.id}>
                    ...
                    <div className="super-admin-log-meta">
                      ...
                      <Badge tone={backup.status === "Success" ? "good" : "danger"}>{backup.status}</Badge>

...
export function updateTrialSettings(payload: { superAdminId: number; trialEnabled: boolean; trialDays: number; backupSchedule: "Manual" | "Daily" | "Weekly"; licenseKey?: string }) {
  const database = getDatabase();
  ...
  database.prepare("INSERT INTO system_logs (super_admin_id, action, details, created_at) VALUES (?, ?, ?, ?)").run(
    ...
    `Trial ${payload.trialEnabled ? "enabled" : "disabled"}, ${trialDays} days, backup ${schedule}, license ${licenseStatus}`,
    now()
  );
...
export function markBackupCreated(superAdminId: number | undefined, details: string) {
  const database = getDatabase();
...
export function createInventoryItem(payload: { actorId: number; categoryId: number; name: string; stock: number; sellPrice: number; supplierId?: number | null }) {
  const database = getDatabase();
  requireInventoryManager(database, payload.actorId);
  const name = payload.name.trim();
...
export function updateInventoryItem(payload: { actorId: number; itemId: number; categoryId: number; name: string; stock: number; sellPrice: number; supplierId?: number | null }) {
  const database = getDatabase();
  requireInventoryManager(database, payload.actorId);
  const item = database.prepare("SELECT id, product_code FROM inventory WHERE id = ?").get(payload.itemId) as { id: number; product_code: string } | undefined;
  ...
  if (!Number.isFinite(stock) || stock < 0) throw new Error("Stock count must be zero or higher.");
  if (!Number.isFinite(sellPrice) || sellPrice < 0) throw new Error("Sell price must be zero or higher.");
  database.prepare("UPDATE inventory SET category_id = ?, category = ?, name = ?, supplier_id = ?, stock = ?, sell_price = ? WHERE id = ?").run(
    category.id,
    category.name,
    name,
...
export function payJobOrder(payload: { actorId: number; jobOrderId: number; paymentMethod: string; paymentReferenceCode?: string }) {
  ...
  const transaction = database.transaction(() => {
    ...
    for (const product of products) reduceStock.run(product.quantity, product.itemId, product.quantity);
    database.prepare(`
      UPDATE job_orders
...
export function createSale(payload: {
  ...
  const transaction = database.transaction(() => {
    ...
    for (const item of payload.items) {
      ...
      if (item.itemType === "part") reduceStock.run(item.quantity, item.itemId, item.quantity);
    }
